...
async function saveHierarchy(hierarchyData, context) {
  ...
  await HierarchyGroup.deleteMany(companyScope);
...
async function deleteUser(userId, context = {}) {
  ...
  return res.deletedCount ? 'success' : 'User not found.';
}
...
async function archiveOldDataCompat(days = 180) {
  ...
  const [delegations, workRequests, checklists] = await Promise.all([
    DelegationTask.countDocuments({ updatedAt: { $lt: cutoff }, status: 'Done' }),
    WorkRequest.countDocuments({ updatedAt: { $lt: cutoff }, status: 'Done' }),
    ChecklistTask.countDocuments({ updatedAt: { $lt: cutoff }, status: 'Done' })
  ]);
