...
async function saveChecklistTask(taskData, userName, context = {}) {
  ...
  const normalizeFrequency = (f) => {
    ...
    const allowed = ['daily', 'weekly', 'fortnightly', 'monthly', 'quarterly', 'yearly'];
...
async function getFilteredDataForCard(cardType, status, userName, userRole, filters = {}, context = {}) {
  ...
  if (card === 'delegation') {
    ...
    if (statusWanted === 'delayed') {
      return delegations.filter((t) => clean(t.taskStatus) === 'completed' && clean(t.onTimeStatus) === 'late');
    }
    return delegations;
  }
  ...
  if (card === 'work request' || card === 'workrequest') {
    ...
    if (statusWanted === 'delayed') {
      return workRequests.filter((t) => clean(t.status) === 'completed' && clean(t.onTimeStatus) === 'late');
    }
    return workRequests;
  }
  ...
  if (card === 'fms') {
    ...
    if (statusWanted === 'delayed') {
      return normalized.filter((t) => Boolean(t.actualDate) && clean(t.onTimeStatus) === 'late');
    }
    return normalized;
  }
